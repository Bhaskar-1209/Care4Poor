...
router.post("/register", async (req, res) => {
  try {
    const { name, email, position, role, password, regards, organizations } = req.body;
...
router.post("/login", async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      email,
      organizations: { $in: [organization] }  // checks if user belongs to the organization
    });
    ...
    const token = jwt.sign(
      { id: user._id, role: user.role, organization },
