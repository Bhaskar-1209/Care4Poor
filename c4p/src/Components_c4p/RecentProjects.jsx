...
const RecentProjects = () => {
  ...
  useEffect(() => {
    const fetchLatestProjects = async () => {
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/api/c4p/projects`);
  ...
  return (
    <div className="bg-[#ebe6e0] py-16 px-6 sm:px-10 md:px-20 relative">
      <div className="flex justify-between items-center mb-10 flex-wrap gap-4">
        <div>
          <p className="text-[#8ba07c] text-[25px] uppercase font-semibold mb-1">Complete Projects</p>
          <h2 className="text-4xl font-bold text-black">Our Recent Projects</h2>
        </div>
        <Link
          to="/more-project"
          className="bg-[#8ba07c] hover:bg-[#8ba07c] text-white px-6 py-2 rounded-full font-medium text-sm transition"
        >
          View All Projects →
        </Link>
      </div>
